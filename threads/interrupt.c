...
/* The Interrupt Descriptor Table (IDT).  The format is fixed by
   ...
   // idt는 인터럽트 256개에 관한 Descriptor(설명,처리방법) 모아둔거
...
intr_handler (struct intr_frame *frame) {
	...
	if (handler != NULL)
		handler (frame); // handler 함수를 통해 timer_interrupt 함수 실행됨 
	...
	if (external) {
		...
		if (yield_on_return) // timer interrupt 함수에서 yield_on_return 값을 1로 변경해서 리턴 
			thread_yield ();
	}
}
