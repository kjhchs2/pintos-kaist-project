...
static int64_t next_tick_to_awake; // sleep_list에서 대기중인 스레드들의 wakeup_tick 값 중 최소값을 저장하는 전역변수
...
void thread_init(void)
{
    ...
    init_thread(initial_thread, "main", PRI_DEFAULT); // 우선순위 부여, 초기화 
    initial_thread->status = THREAD_RUNNING; 
    initial_thread->tid = allocate_tid(); //tid값 + 1
}
...
void thread_start(void)
{
    ...
    intr_enable(); // 인터럽트 시작함수 (타이머 시작)

    /* Wait for the idle thread to initialize idle_thread. */
    sema_down(&idle_started); // 스레드 교환 (실행->대기)
}
...
void thread_tick(void)
{
    ...
    /* Update statistics. */
    // ide_thread 가 실행중이면 idle_tick을 올려준다 
    if (t == idle_thread)
        idle_ticks++; 
#ifdef USERPROG
    else if (t->pml4 != NULL)
        user_ticks++;
#endif
    // 아니라면 커널 스레드가 실행중인거니까 kernel_tick 올려준다 
    else
        kernel_ticks++;
...
/* Creates a new kernel thread named NAME with the given initial
   ...
   // 이 함수가 하는 일 
   // 1. 스레드 할당, 초기화 2. switch_thread와 kernel_thread를 위한 가짜 스택 프레임 생성
   // function : running될때 실행되는 함수 
...
/* Puts the current thread to sleep.  It will not be scheduled
   ...
   // thread_unblock이 호출될때까지 스케줄되지 않는다.
...
idle(void *idle_started_ UNUSED)
{
    ...
    idle_thread = thread_current(); // idle_thread 전역화 !!!!!!!!! 
...
schedule(void)
{
    ...
    ASSERT(intr_get_level() == INTR_OFF); // 인터럽트 꺼졌는지 확인
    ASSERT(curr->status != THREAD_RUNNING); // 현재 스레드 상태 바꿨는지 확인 
    ASSERT(is_thread(next)); // 다음 실행할 스레드가 멀쩡한 앤지 확인 
    /* Mark us as running. */
    ...
    if (curr != next) // 만약 curr == next 면(idle_thread) 문맥 교환 안해도되니까 != 
    {
...
void thread_sleep(int64_t ticks)
{
    ...
    ASSERT(!intr_context()); // 인터럽트에 반응중이니까 이거 false인데 실행된거면 잘못된거니까 에러 발생시키기

    // 해당 과정 중에는 인터럽트를 받아들이지 않는다
    // 중간에 인터럽트가 발생하면 예기치 못한 문제가 발생할 수 있으므로 현재 스레드가 독점 실행될 수 있도록 함
    old_level = intr_disable();
    ASSERT(curr != idle_thread);

    curr->wakeup_tick = ticks;
    update_next_tick_to_awake(curr->wakeup_tick);         // 새로 들어온 값과 기존의 값 중 더 작은 값으로 next_tick_to_awake 갱신
    list_push_back(&sleep_list, &curr->elem); // curr의 elem을 sleep_list에 넣음
    //do_schedule(THREAD_BLOCKED);   이걸로 해도 되고 thread_block() 해도됨 
    thread_block();
    intr_set_level(old_level);
}

// thread_awake 함수 구현
// 인자로 전달된 tick이 현재 tick
// 현재 tick >= thread's wakeup_tick : unblock(깨움)
// 아니면, update_next_tick_to_awake
void thread_awake(int64_t ticks)
{
    next_tick_to_awake = INT64_MAX;
    struct list_elem *check_ptr = list_begin(&sleep_list);

    while (check_ptr != list_end(&sleep_list))
    {
        struct thread *th = list_entry(check_ptr, struct thread, elem);
        if (ticks >= th->wakeup_tick)
        {
            check_ptr = list_remove(&th->elem);
            thread_unblock(th);
        }
        else
        {
            check_ptr = list_next(check_ptr);
...
{
    // 원래 값과 인자로 받은 새 값 중 더 작은 값으로 갱신
